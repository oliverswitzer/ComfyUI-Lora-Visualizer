...
function drawButton(ctx, x, y, width, height, text, backgroundColor, borderColor) {
...
function drawLoRAText(ctx, lora, x, startY) {
    ...
    const nameText = lora.name.length > 25 ? lora.name.substring(0, 25) + "..." : lora.name;
...
function drawLoRAButtons(ctx, lora, textX, triggerY, node) {
    ...
    // Copy button
    drawButton(ctx, textX, copyButtonY - 10, 120, 16, "Copy Trigger Words", "#555", "#777");
    ...
    if (metadata?.civitaiUrl) {
        ...
        drawButton(ctx, linkButtonX, copyButtonY - 10, 80, 16, "Open Civitai", "#2196F3", "#1976D2");
...
function handleMouseMove(e, node) {
    ...
    if (hoveredThumbnail && hoveredThumbnail !== node.currentlyHovered) {
        ...
        const thumbSize = Math.min(Math.max(node.size[0] * 0.20, 60), 150);
        showHoverGallery(hoveredThumbnail.lora, e.clientX, e.clientY, thumbSize, node);
...
function addGalleryImages(gallery, metadata, thumbnailSize) {
    if (!metadata.exampleImages?.length) {
        ...
        noImagesDiv.style.cssText = 'margin-top: 10px; font-size: 12px; color: #999;';
...
function drawLoRAVisualization(ctx, width, y, height, node) {
    ...
    if (standardLoras.length > 0) {
        currentY = drawLoRASection(ctx, "Standard LoRAs", standardLoras, currentY, width, "#4a9eff", node);
    }
    
    if (wanloras.length > 0) {
        currentY = drawLoRASection(ctx, "WanLoRAs", wanloras, currentY, width, "#ff9a4a", node);
    }
...
function drawLoRASection(ctx, title, loras, startY, width, accentColor, node) {
    ...
    loras.forEach(lora => {
        ...
        currentY = drawLoRAItem(ctx, lora, 20, currentY, width - 30, accentColor, node);
    });
...
function createVisualizationWidget(node) {
    const widget = node.addWidget("LORA_VIZ", "lora_visualization", "", () => {}, {
        serialize: false
    });
...
function setupWebSocketHandler() {
    api.addEventListener("lora_visualization_data", (event) => {
        ...
        if (messageData.node_id) {
            const targetNode = app.graph._nodes.find(n => n.id.toString() === messageData.node_id);
