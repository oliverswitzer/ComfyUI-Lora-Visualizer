  ...
  async beforeRegisterNodeDef(nodeType, nodeData, app) {
    if (nodeData.name === "LoRAVisualizer") {
      ...
      nodeType.prototype.onResize = function (size) {
        ...
        // Canvas buttons redraw automatically with the canvas
      ...
      nodeType.prototype.addMouseEventHandlers = function () {
        ...
        canvas.addEventListener("mousemove", (e) => {
          ...
          this.mouseTimeout = setTimeout(() => {
            ...
            // Don't update button positions on mouse move - only on pan/zoom
          }, 16); // ~60fps
        });
      ...
      // Canvas click handling for button areas
      ...
      nodeType.prototype.showCopyFeedback = function (
        buttonArea,
        isError = false
      ) {
        ...
        const buttonScreenX =
          rect.left + (buttonArea.x + nodeRect[0] + offsetX) * scale;
        const buttonScreenY =
          rect.top + (buttonArea.y + nodeRect[1] + offsetY) * scale;
      ...
      nodeType.prototype.showHoverGallery = function (
        lora,
        x,
        y,
        thumbnailSize = 80
        ...
        const metadata =
          this.loraMetadataCache && this.loraMetadataCache[loraKey];
        ...
        gallery.style.cssText = `
                    ...
                    border: 2px solid ${
                      lora.type === "wanlora" ? "#ff9a4a" : "#4a9eff"
                    };
                    border-radius: 8px;
                    padding: 15px;
                    max-width: 400px;
      ...
      nodeType.prototype.drawLoRAVisualization = function (
        ctx,
        width,
        y,
        height
      ) {
        // Clear thumbnail areas for this redraw
        this.thumbnailAreas = [];
        this.currentThumbnailSize = null;

        // Only clear button areas and buttons if data has actually changed
        const dataKey = JSON.stringify({
          standard: this.loraData.standard_loras || [],
          wanloras: this.loraData.wanloras || [],
        });

        // Clear button areas for this redraw
        this.buttonAreas = [];
        ...
        if (standardLoras.length > 0 || wanloras.length > 0) {
          console.debug(
            `Drawing LoRAs - Standard: ${standardLoras.length}, WanLoRAs: ${wanloras.length}`
          );
        }
        ...
        if (standardLoras.length > 0) {
          ...
          ctx.fillText(
            `Standard LoRAs (${standardLoras.length})`,
            10,
            currentY
          );
          currentY += 20;
        ...
        if (
          this.loraVisualizationWidget &&
          neededHeight !== this.loraVisualizationWidget.computeSize(width)[1]
        ) {
        ...
        // Canvas-drawn buttons are handled in the drawing code itself
      };

      // Add method to draw individual LoRA item
      nodeType.prototype.drawLoRAItem = function (
        ctx,
        lora,
        x,
        y,
        ...
        const thumbSize = Math.min(Math.max(width * 0.2, 60), 150); // Min 60px, max 150px, 20% of width
        const itemHeight = Math.max(thumbSize + 20, 130); // Minimum height for larger text and button row
        ...
        if (lora.triggerWords && lora.triggerWords.length > 0) {
          ...
          // Copy button on its own line (canvas-drawn)
          const copyButtonY = triggerY + 16;
          ctx.fillStyle = "#555";
          ctx.fillRect(textX, copyButtonY - 10, 120, 16);
          ctx.strokeStyle = "#777";
          ctx.strokeRect(textX, copyButtonY - 10, 120, 16);

          ctx.fillStyle = "#fff";
          ctx.font = "10px Arial";
          ctx.fillText("Copy Trigger Words", textX + 4, copyButtonY);

          // Store click area for copy button
          if (!this.buttonAreas) {
            this.buttonAreas = [];
          }
          ...
          // Civitai link button next to copy button
          const loraKey = `${lora.name}_${lora.type}`;
          const metadata =
            this.loraMetadataCache && this.loraMetadataCache[loraKey];
          if (metadata && metadata.civitaiUrl) {
            const linkButtonX = textX + 125; // Adjusted for wider copy button
            ctx.fillStyle = "#2196F3";
            ctx.fillRect(linkButtonX, copyButtonY - 10, 80, 16);
            ctx.strokeStyle = "#1976D2";
            ctx.strokeRect(linkButtonX, copyButtonY - 10, 80, 16);

            ctx.fillStyle = "#fff";
            ctx.font = "10px Arial";
            ctx.fillText("Open Civitai", linkButtonX + 4, copyButtonY);
        ...
        this.htmlButtonAreas.forEach((buttonInfo, index) => {
          ...
          button.style.cssText = `
                position: fixed;
                        ...
                        height: 16px;
                        border: 1px solid ${
                          buttonInfo.type === "copy" ? "#777" : "#1976D2"
                        };
                        background: ${
                          buttonInfo.type === "copy" ? "#555" : "#2196F3"
                        };
                        color: white;
                        font-size: 10px;
                        padding: 0;
          ...
          const screenX =
            canvasRect.left + (buttonInfo.x + nodeRect[0] + offsetX) * scale;
          const screenY =
            canvasRect.top + (buttonInfo.y + nodeRect[1] + offsetY) * scale;
          ...
          if (buttonInfo.type === "copy") {
            button.onclick = () =>
              this.copyTriggerWords(buttonInfo.triggerWords);
      ...
      nodeType.prototype.copyTriggerWords = function (triggerWords) {
        ...
        if (navigator.clipboard && navigator.clipboard.writeText) {
          ...
          try {
            ...
            console.debug(
              "Trigger words copied to clipboard (fallback):",
              triggerText
            );
            this.showCopyFeedback("Copied to clipboard!");
          } catch (err) {
      ...
      nodeType.prototype.loadAndDrawThumbnail = function (
        ctx,
        lora,
        x,
        y,
        ...
        try {
          ...
          if (response.ok) {
            ...
            if (
              ...
              metadata.civitai.images.length > 0
            ) {
              ...
              if (thumbnailEntry) {
                ...
                if (isVideo) {
                  // Use the same approach as hover gallery - create actual video element
                  console.debug(`Loading video thumbnail for ${lora.name}`);
                  ...
                  video.autoplay = true;
                  video.loop = false; // Don't loop for thumbnail capture

                  // Use loadeddata event instead of onplaying
                  video.addEventListener("loadeddata", () => {
                    console.debug(`Video loadeddata event for ${lora.name}`);
                  ...
                  video.addEventListener("seeked", () => {
                    console.debug(
                      `Video seeked for ${lora.name}, capturing frame`
                    );
                  ...
                  video.addEventListener("error", () => {
                    console.warn(
                      `Failed to load video thumbnail for ${lora.name}`
                    );
                    this.loraMetadataCache[loraKey] = processedMetadata;
                    this.setDirtyCanvas(true, true);
                  });
                  ...
                  img.onerror = () => {
                    console.warn(
                      `Failed to load image thumbnail for ${lora.name}`
                    );
                    this.loraMetadataCache[loraKey] = processedMetadata;
                    this.setDirtyCanvas(true, true);
                  };
